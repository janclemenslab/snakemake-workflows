"""
Module for creating a form from a dict or yaml file.

Example:

>>> widget = DictFormWidget.from_yaml(yaml_file="example.yaml")
...
class DictFormWidget(QtWidgets.QGroupBox):
    """
    Custom QWidget which creates form based on dictionary.

    Args:
        for_dict: dict with form data
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(DictFormWidget, self).__init__(*args, **kwargs)

        items_to_create = form_dict
    ...
    @classmethod
    def from_yaml(cls, yaml_file: str, Loader=yaml.SafeLoader):
        with open(yaml_file, "r") as form_yaml:
            form_dict = yaml.load(form_yaml, Loader=Loader)
...
class FormBuilderModalDialog(QtWidgets.QDialog):
    ...
    def __init__(
        self, form_widget: DictFormWidget, *args, **kwargs,
    ):
